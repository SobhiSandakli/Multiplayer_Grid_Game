...
describe('SaveService', () => {
    ...
    it('should call gameFacade.createGame when there is no gameId', async () => {
        ...
        await Promise.resolve(); // Wait for the promise chain to complete
    ...
    // Test to cover input length exceeding max value for name and description
    it('should show snack bar when game name exceeds max length', () => {
        const event = { target: { value: 'A'.repeat(NAME_MAX_LENGTH + 1) } } as unknown as Event; // Exceeding the 30 character limit
    ...
    it('should show snack bar when game description exceeds max length', () => {
        const event = { target: { value: 'A'.repeat(DESCRIPTION_MAX_LENGTH + 1) } } as unknown as Event; // Exceeding the 100 character limit
    ...
    it('should not proceed with saving if game name or description is missing', () => {
        service.onSave('', 'Test Description'); // Missing game name
        expect(snackBarSpy.open).toHaveBeenCalledWith('Veuillez remplir le nom et la description du jeu.', 'OK', { duration: 5000 });

        service.onSave('Test Game', ''); // Missing game description
        expect(snackBarSpy.open).toHaveBeenCalledWith('Veuillez remplir le nom et la description du jeu.', 'OK', { duration: 5000 });

        service.onSave('', ''); // Both missing
        expect(snackBarSpy.open).toHaveBeenCalledWith('Veuillez remplir le nom et la description du jeu.', 'OK', { duration: 5000 });
    });
    ...
    // Test for handling duplicate game name error
